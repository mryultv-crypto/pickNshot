...
def get_gemini_response(content, vibe):
    ...
    # 이미지와 텍스트 포맷팅
    # gemini-pro-vision은 리스트 형태 [프롬프트, 이미지]를 받습니다.
    final_content = [system_instruction, content[0]]
    
    try:
        # [수정됨] 1.5-flash를 아예 삭제하고, pro-vision으로 고정
        model = genai.GenerativeModel('gemini-pro-vision')
        response = model.generate_content(final_content)
        return response.text, "Gemini Pro Vision (Classic)"
    except Exception as e:
        # 만약 이것도 안 되면 텍스트 전용 모델이라도 시도 (이미지 제외)
        return f"Error: {str(e)}\n\n(Tip: 구글 API 키가 정확한지, 결제 계정이 연결되어 있는지 확인해보세요.)", "Fail"
